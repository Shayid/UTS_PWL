...
$ay_agama = ['Islam', 'Kristen Katholik', 'Kristen Protestan', 'Hindu', 'Budha', 'Khonghucu'];
$obj = new Pegawai();
$rs = $obj->dataDivisi();
    ...
    <div class="form-group row">
        ...
        <div class="col-8">
            <select id="divisi" name="divisi" class="custom-select" required="required">
                ...
                foreach ($rs as $s) {
                    //edit divisi
                    $sel_divisi = ($edit_data['iddivisi'] == $s['id']) ? "selected" : "";
                ?>
                    <option value="<?= $s['id'] ?>" <?= $sel_divisi ?>> <?= $s['nama'] ?> </option>
                <?php } ?>
            </select>
        </div>
    </div>
